...
#define CMD_IMG     1
    ...
    uart_putbuff(Camera_Uart, cmdf, sizeof(cmdf));    //先发送命令

    uart_putbuff(Camera_Uart, (uint8_t *)imgaddr, imgsize); //再发送图像

    uart_putbuff(Camera_Uart, cmdr, sizeof(cmdr));    //先发送命令
    
    //uart_putchar(Camera_Uart,0X10);
}
...
#define CMD_CCD     2
    ...
    uart_putbuff(Camera_Uart, (uint8_t *)ccdaddr, ccdsize); //再发送图像

    uart_putbuff(Camera_Uart, cmdr, sizeof(cmdr));    //再发送命令
}
...
#define CMD_WARE     3
    ...
    uart_putbuff(Camera_Uart, cmdf, sizeof(cmdf));    //先发送前命令
    uart_putbuff(Camera_Uart, (uint8_t *)wareaddr, waresize);    //发送数据
    uart_putbuff(Camera_Uart, cmdr, sizeof(cmdr));    //发送后命令
}